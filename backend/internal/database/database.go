...
import (
	...
	loggerConfig "github.com/047pegasus/go-boilerplate/internal/logger"
...
func New(cfg *config.Config, logger *zerolog.Logger, ls *loggerConfig.LoggerService) (*Database, error) {
	...
	// Add Sentry PostgreSQL instrumentation integration
	if ls != nil && ls.GetApplication() != nil {
		pgxPoolConfig.ConnConfig.Tracer = NewSentryTracer()
	}

	//setting tracers based on environemnt of execution
	if cfg.Primary.Env == "local" {
		globalLevel := logger.GetLevel()
		pgxLogger := loggerConfig.NewPgxLogger(globalLevel)
		//chain the tracers, sebtry first and then local logging
		if pgxPoolConfig.ConnConfig.Tracer != nil {
			//if Sentry tracer exists we create a multitracer
	...
	if err != nil {
		return nil, fmt.Errorf("failed to connect to database, failed to create pgx pool: %w", err)
	}
	database := &Database{
		Pool: pool,
		log:  logger,
	...
	logger.Info().Msgf("successfully connected to database: %s", cfg.Database.Name)
