...
import (
	...
	"github.com/047pegasus/go-boilerplate/internal/validation"
)
...
type ResponseHandler interface {
	...
	GetOperation() string
	AddAttributes(txn *apm.Transaction, result interface{})
}
...
func (h JSONResponseHandler) AddAttributes(txn *apm.Transaction, result interface{}) {
...
func (h NoContentResponseHandler) AddAttributes(txn *apm.Transaction, result interface{}) {
	// http.status_code is already set by tracing middleware
}
...
func (h FileResponseHandler) AddAttributes(txn *apm.Transaction, result interface{}) {
	if txn != nil {
		// http.status_code is already set by tracing middleware
...
func handleRequest[Req validation.Validatable](
	...
	// Get Sentry transaction from context
	txn := apm.FromContext(c.Request().Context())
	if txn != nil {
		txn.AddAttribute("handler.name", route)
		// http.method and http.route are already set by sentryecho + EnhanceTracing middleware
	...
	if err := validation.BindAndValidate(c, req); err != nil {
		...
		if txn != nil {
			txn.NoticeError(apm.WrapError(err))
